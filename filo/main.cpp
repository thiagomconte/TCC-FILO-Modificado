...
auto main(int argc, char *argv[]) -> int
{
    ...
    const auto outfile = arg_parser.get_outpath() + get_basename(arg_parser.get_instance_path()) + "_seed-" + std::to_string(arg_parser.get_seed()) + ".out";
    ...
    clock_t global_time_clock = clock();
    ...
    if (parser_type == Z_PARSER || parser_type == K_PARSER)
    {
        round_costs = false;
    }

    auto maybe_instance = parser_type == X_PARSER ? cobra::Instance::make<cobra::XInstanceParser, true>(arg_parser.get_instance_path()) : (parser_type == Z_PARSER ? cobra::Instance::make<cobra::ZKInstanceParser, false>(arg_parser.get_instance_path()) : cobra::Instance::make<cobra::KytojokiInstanceParser, false>(arg_parser.get_instance_path()));
    ...
    auto rvnd0 = cobra::RandomizedVariableNeighborhoodDescent(instance, move_generators, {cobra::E11, cobra::E10, cobra::TAILS, cobra::SPLIT, cobra::RE22B, cobra::E22, cobra::RE20, cobra::RE21, cobra::RE22S, cobra::E21, cobra::E20, cobra::TWOPT, cobra::RE30, cobra::E30, cobra::RE33B, cobra::E33, cobra::RE31, cobra::RE32B, cobra::RE33S, cobra::E31, cobra::E32, cobra::RE32S}, rand_engine, tolerance);
    ...
    for (auto iter = 0; iter < coreopt_iterations; iter++)
    {
        ...
        if (std::chrono::duration_cast<std::chrono::minutes>(
                std::chrono::high_resolution_clock::now() - global_time_begin)
                .count() >= elapsed_minutes + 5)
        {
        ...
        if (iter % 100 == 0)
        {
            renderer.draw(best_solution, neighbor.get_cache(), move_generators);
        }
#endif
    ...
    global_time_clock = clock() - global_time_clock;
    ...
    auto outcsv = arg_parser.get_outpath() + get_basename(arg_parser.get_instance_path()) + "_seed-" + std::to_string(arg_parser.get_seed()) + ".csv";
    out_stream = std::ofstream(outcsv);
    out_stream << "Instancia;"
               << "Total de nós;"
               << "Valor da solução;"
               << "Tempo total;"
               << "Tempo total Clock();"
               ...
               << "Tempo Ordenação"
               << "\n";
    out_stream << arg_parser.get_instance_path()
               << ";" << instance.get_demandSize()
               << ";" << best_solution.get_cost()
               << ";" << std::chrono::duration_cast<std::chrono::seconds>(global_time_end - global_time_begin).count()
               << ";" << std::to_string(((float)global_time_clock) / CLOCKS_PER_SEC)
               << ";" << std::to_string(((float)instance.get_tempoAPI()) / CLOCKS_PER_SEC)
               << ";" << std::to_string(((float)instance.get_tempoSort()) / CLOCKS_PER_SEC);
